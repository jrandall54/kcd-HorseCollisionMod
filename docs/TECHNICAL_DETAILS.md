...
and the packaged release. It confirms that no vanilla filename is claimed,
nothing is dropped from the fragment the mod takes over, every reference in the
chain resolves, pak entry names use forward slashes, and the Lua redirects the
classes the engine spawns. It discovers those classes by reading `Scripts.pak`
instead of taking the mod's word for them. Run it before publishing.
