...
-- and every one of them is named `hcm_*` so it cannot collide with another
-- mod:
--
-- * `hcm_<gender>_database.adb` - a few hundred bytes holding no fragments
--   of its own. It references the vanilla database, untouched inside its own
--   pak, and the stagger file below. `RedirectAnimationDatabases` points the
--   human entity classes at it.
-- * `hcm_<gender>_stagger.adb` - the `hcm_stagger_*` options, pointing at
--   `hitreaction_idle_medium_torso_stab_*`, a standing hit reaction the game
--   already contains.
-- * `hcm_<gender>_fragmentids.xml` - points `AnimationControlled` at the tag
--   file below. For women it also has to declare that fragment at all, since
--   they have the same clips but no such fragment in vanilla.
